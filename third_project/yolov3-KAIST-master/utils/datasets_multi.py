...
class LoadImages:  # for inference
    def __init__(self, path, nchannel, img_size=416):
        ...
        if os.path.isdir(path):
            if nchannel == 4:
                RGB_file = sorted(glob.glob(os.path.join(RGB_path, '*.jpg*')))
                IR_file = sorted(glob.glob(os.path.join(IR_path, '*.jpg*')))
            else:
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path_rgb, path_ir, nchannel=3, img_size=416, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        if not labels_loaded and n > 1000:
            ...
            ########################
            ########################
            #LINEA AGGIUNTA DA NOI
            concatenated_labels = np.vstack(self.labels) 
            #################################################
            ################################################
            np.save(np_labels_path, concatenated_labels)  # save for next time
